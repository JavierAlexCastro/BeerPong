...
public class Tables extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Button texas_btn = (Button) findViewById(R.id.button7);
        final Button khalili_btn = (Button) findViewById(R.id.button8);
        final Button back_btn = (Button) findViewById(R.id.back);
        back_btn.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v) {
                Intent about = new Intent(Tables.this, Shop.class);
                startActivity(about);
        ...
        uta_btn.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v) {
                SharedPreferences settings = getApplicationContext().getSharedPreferences("PREFS_NAME", Context.MODE_PRIVATE);
                if(settings.getBoolean("UTATABLE", false)) { //already purchased
                    showPurchasedImage();
                } else{
                    ...
                    if(creditAmount<50) {
                        ...
                        creditAmount -= 50;
                        editor.putBoolean("UTATABLE", true);
        ...
        texas_btn.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v) {
                SharedPreferences settings = getApplicationContext().getSharedPreferences("PREFS_NAME", Context.MODE_PRIVATE);
                if(settings.getBoolean("TEXASTABLE", false)) { //purchased before
                    showPurchasedImage();
                } else{
                    int creditAmount = settings.getInt("CREDITS", Context.MODE_PRIVATE);
                    if (creditAmount < 100) {
                        ...
                        SharedPreferences.Editor editor = settings.edit();
                        creditAmount -= 100;
                        editor.putBoolean("TEXASTABLE", true);
                        editor.putInt("CREDITS", creditAmount);
                        editor.apply();
                        amount.setText(String.valueOf(creditAmount));
                        showSuccessImage();
        ...
        khalili_btn.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v) {
                SharedPreferences settings = getApplicationContext().getSharedPreferences("PREFS_NAME", Context.MODE_PRIVATE);
                if(settings.getBoolean("KHALILITABLE", false)) { //purchased before
                    showPurchasedImage();
                } else {
                    int creditAmount = settings.getInt("CREDITS", Context.MODE_PRIVATE);
                    if (creditAmount < 250) {
                        ...
                        SharedPreferences.Editor editor = settings.edit();
                        creditAmount -= 250;
                        editor.putBoolean("KHALILITABLE", true);
                        editor.putInt("CREDITS", creditAmount);
                        editor.apply();
                        amount.setText(String.valueOf(creditAmount));
                        showSuccessImage();
